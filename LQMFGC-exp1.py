...
def solve_experiment_1(T=1.0, X0=1.0, lams=None, dt=0.001):
    ...
    for lam in lams:
        ...
        for i in range(steps - 1, -1, -1):
            ...
            const_term = 0.5 - lam * (1 - lam)
            dPi = Pi[i+1]**2 - P[i+1] - const_term
            Pi[i] = Pi[i+1] - dPi * dt

        # The corrected equilibrium equations form a two-point boundary-value
        # problem: z(0) = X0 and phi(T) = 0. Since the system is linear, use a
        # scalar shooting step to determine the unknown value phi(0).
        def integrate(phi0):
            z = np.zeros(steps + 1)
            ...
            for i in range(steps):
                ...
                dphi = (Pi[i] * phi[i]
                        - 0.5 * (1 - lam) * (2 * lam - 1) * z[i])
                phi[i+1] = phi[i] + dphi * dt

            return z, phi

        z0, phi0 = integrate(0.0)
        z1, phi1 = integrate(1.0)
        initial_phi = -phi0[-1] / (phi1[-1] - phi0[-1])
        z, phi = integrate(initial_phi)
        ...
        # Compute the stated equilibrium cost directly. At equilibrium the
        # equilibrium state cost is 0.5 E[X_t^2]. Its variance V satisfies
        # the scalar ODE below, and E[alpha_t^2] = P_t^2 V_t + a_t^2.
        variance = np.zeros(steps + 1)
        for i in range(steps):
            dvariance = (1 - 2 * P[i]) * variance[i] + z[i]**2
            variance[i+1] = variance[i] + dvariance * dt
        running_cost = (P**2 * variance + a**2
                        + 0.5 * (variance + z**2))
        costs.append(np.trapz(running_cost, t_grid))
